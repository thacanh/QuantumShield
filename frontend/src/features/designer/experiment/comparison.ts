...
export interface ComparisonSnapshot {
  ...
  data: { id: string; route: string; protection: string; application: string; transmission: string }[];
}
  ...
  return {
    ...
    sessions: result.sessions.map(s => {
      ...
      return { id: s.sessionId, label: config.participantIds.map(name).join(' ↔ '), protocol: s.protocol,
        ...
        }), sifted: s.siftedCount, siftRatio: s.siftRatio, qber: s.qber,
    ...
    data: (result.dataResults ?? []).map(d => {
      ...
      return { id: d.edgeId, route: `${name(edge.source)} → ${name(edge.target)}`, protection: d.protection, application: d.applicationStatus, transmission: d.transmissionStatus };
    }),
  };
}
