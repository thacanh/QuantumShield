...
export default function ComparisonPanel({ result }: { result?: ExperimentResult }) {
  ...
  return <section className="lab-results" aria-label="So sánh lần chạy A/B">
    ...
    {!!ids.length && <div className="lab-table-scroll"><table aria-label="Bảng so sánh kết quả">
      ...
      <tbody>
        ...
        <tr><th>Trạng thái dữ liệu</th>{(['A', 'B'] as const).map(slot => <td key={slot}>{slots[slot]?.data.map(d => <p key={d.id}>{d.route} · {d.protection === 'none' ? 'Không mã hóa' : vi(d.protection)} · {vi(d.application)} / {vi(d.transmission)}</p>) ?? '—'}</td>)}</tr>
      </tbody>
    </table></div>}
  </section>;
}
